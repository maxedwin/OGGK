<?php
	...
	class Sunat extends Model
	{
		...
		function consultaRuc( $ruc )
		{
			...
			{
				$patron='/<h4 class="list-group-item-heading">(.*)<\/h4>/';
				$output = preg_match_all($patron, $response, $matches, PREG_SET_ORDER);

				if(isset($matches[1]))
				{	
					$RS = trim( substr($matches[1][1], strpos($matches[1][1], '-')+1));
					...
					$patron='/<div[ ]*class="col-sm-7">[\r\n\t[ ]+]*(.*)[\r\n\t[ ]+]*<\/div>/';
					$output = preg_match_all($patron, $response, $matches, PREG_SET_ORDER);
					if (isset($matches[0])) 
					{
						$rtn['departamento'] = trim (utf8_encode(str_replace('</div>','', str_replace('"','', ($matches[4][1])))));
						$rtn['provincia'] = trim (utf8_encode(str_replace('</div>','', str_replace('"','', ($matches[5][1])))));
						$rtn['distrito'] = trim (utf8_encode(str_replace('</div>','', str_replace('"','', ($matches[6][1])))));
						$rtn['direccion'] = trim (utf8_encode(str_replace('</p>','', str_replace('<p class="list-group-item-text">','', ($matches[7][1])))));
						if ($rtn['departamento'] != '-') {
							$rtn['direccion'] .= ' ' . $rtn['departamento'];
						}
						if ($rtn['provincia'] != '-') {
							$rtn['direccion'] .= ' - ' . $rtn['provincia'];
						}
						if ($rtn['distrito'] != '-') {
							$rtn['direccion'] .= ' - ' . $rtn['distrito'];
						}
					}
		...
		{
			$url = "http://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS03Alias";
			$data = array(
				"accion" 	=> "getRepLeg",
				"nroRuc" 	=> $ruc,
				"desRuc" 	=> ""
