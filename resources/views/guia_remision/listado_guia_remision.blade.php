...
@section('titulo')
    <h4><i class="glyphicon glyphicon-list position-left"></i> <span class="text-semibold">Listado de Guías de Remisión</span></h4>
@stop
...
<script type="application/javascript" rel="script">
    ...
    table = $('#cliente_table').DataTable({
            ...
            "ajax":{
                   ...
                   "stateSave": true,
                    "stateSaveParams": function (settings, data) { 
                        ...
                        a.href = currentLocation+"exportGR?order="+set.order[0][0]+"&dir="+set.order[0][1]+"&search="+set.search.search+"&f_inicio="+$('#f_inicio').val()+"&f_fin="+$('#f_fin').val();
        ...
        "paging": true,
        "searching": true,
        "columnDefs" : [{"targets":3, "type":"date-eu"},{"targets":5, "type":"date-eu"},{"targets":6, "type":"date-eu"},{"targets":9, "visible":false}],
        "order": [[ 3, "desc" ], [0, "desc"]],
    ...
    $('#cliente_table').on('click','#lotes',function(events){
        var id = $(this).data('id');     
        var direccion="lotes_guia_remision?id="+ id;
        console.log(currentLocation+direccion);
        window.open(currentLocation+direccion,  "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,,200,width=1000,height=500");
    });
    });
