            ...
            <main>
                ...
                <table border="0" cellspacing="0" cellpadding="0">
                    <thead>
                        <tr>
                            ...
                            <th>UNID.</th>
                            <th>DESCRIPCIÓN</th>
                            <th>V. UNITARIO <?PHP if($cotizacion->moneda == 1){ echo "(S/.)"; }else if($cotizacion->moneda == 2){ echo "($.)"; } else{ echo "(€.)"; }?></th>
                            <th>V. TOTAL <?PHP if($cotizacion->moneda == 1){ echo "(S/.)"; }else if($cotizacion->moneda == 2){ echo "($.)"; } else{ echo "(€.)"; }?></th>
                        </tr>
                    </thead>
                    <tbody>
                    @for ($i = 0; $i < count($cotizacionD); $i++)
                            <tr>
                                <td class="text-center"> {{ $i+1 }}                               </td>
                                <td class="text-center"> {{ $cotizacionD[$i]->cantidad }}       </td>
                                <td class="text-center"> {{ $cotizacionD[$i]->medida_venta }}   </td>
                                <td class="text-left">   {{ $cotizacionD[$i]->nombre }}         </td>
                                <td class="text-right">  {{ sprintf("%.2f",round($cotizacionD[$i]->precio_unit,2)) }} </td>
                                <td class="text-right">  {{ sprintf("%.2f",round($cotizacionD[$i]->precio_unit,2) * $cotizacionD[$i]->cantidad) }}</td>
                            </tr>
                    @endfor
                    </tbody>
                    <tfoot>
                        ...
                        <!--tr>
                            <td colspan="2"></td>
                            <td colspan="3">DTO.</td>
                            <td>{{ sprintf("-%.2f",round($cotizacion->descuento,2)) }}</td>
                        </tr-->
                        <tr>
                            <td colspan="2"></td>
                ...
                <div class="row notices">
                    <div class="col notice">  
                        ...
                        Precios:                <span class="tab">En <?PHP if($cotizacion->moneda == 1){ echo "Soles"; }else if($cotizacion->moneda == 2){ echo "Dolares Americanos"; } else{ echo "Euros"; }?>, los valores unitarios no incluyen IGV.</span><br>
                        ...
                        Observaciones <span class="tab">{{ $cotizacion->comentarios  }}</span><br><br>
                    </div>
                </div>
