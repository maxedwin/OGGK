...
@section('titulo')
    <h4><i class="glyphicon glyphicon-usd position-left"></i> <span class="text-semibold">Crear Factura de Compra</span></h4>
@stop

<!--BREADCRUMB -->
@section('breadcrumb')
    <li><a href="/"><i class="icon-home2 position-left"></i> Página Principal</a></li>
    <li class=""><a href="/listado_factura_compra"></i>Listado de Facturas de Compra</a></li>
    <li class="active">Crear Factura de Compra </li>
@stop
<!-- MENU AUXLIAR -->
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group form-inline">
                            ...
                            <div class="input-group ">
                                ...
                                <input type="date" class="form-control" id="f_vencimiento" >
                            </div>
                            <div class="input-group ">
   ...
   <script>
            ...
            if (event.which == 13 || event.keyCode == 13) {
                ...
                $.get(currentLocation+"buscarProductoCoti?query="+query+"", function( data ) {
                    ...
                    $.each(productos, function(key,value){
                            ...
                            string += '<td>'+ parseFloat(value.costo).toFixed(2)+'</td>';
        ...
        $('#lista').on('click','#btn_agregar',function(event){
            ...
            if (checkIdProducSelect(idproducto)) {
                ...
                string += '<td id="costo_unitario"><input class="form-control" type="text" id="costo_prod" value="'+ parseFloat(producto.costo).toFixed(2) +'"></td>';
        ...
        $('#factbol_search').keyup(function(event) {
            var query = $(this).val();
            if(query.length >  0){
                $.get(currentLocation+"buscarOCNumRecv?query="+query+"", function( data ) {
        ...
        $( "#search_results2" ).on( "click","#item_to_add2", function() {
            ...
            $.get(currentLocation+"buscarOCTodoFact?query="+query+"", function( data ) {
                var obj = JSON.parse(data);
                ...
                $.each(obj.detalle, function(index, producto) {
                    ...
                    string += '<td id="detalle_cantidad"><input class="form-control" id="input_cantidad" data-idprod="'+ producto.idproducto +'" type="number" step="1" value="'+producto.cantidad+'" ></td> ';
                     string += '<td id="costo_unitario"><input class="form-control" type="text" id="costo_prod" value="'+ parseFloat(producto.costo_unit)+'"></td>';
                    string += '<td id="detalle_total"><input class="form-control" id="input_costo" type="number" step="0.1" value="'+producto.costo_total+'" disabled></td> ';
                    string += '<td><button class="btn btn-danger btn-xs" id="eliminar" data-idprod="'+ producto.idproducto +'" ><i class="glyphicon glyphicon-remove"></i></button></td> ';
                ...
                if (obj.facturas.length > 0) {
                    var $moneda = {1:"Soles",2:"Dolares",3:"Euros"};
                    var $estado = {0:'<button class="btn btn-danger btn-xs">Pendiente</button>',1:'<button class="btn btn-danger btn-xs">Pendiente</button>',2:'<button class="btn btn-success btn-xs">Cancelada</button>',3:'<button class="btn btn-secondary btn-xs">Anulada</button>'};
                    $.each(obj.facturas, function(index, factura){

                            string += '<tr>';
                            string += '<td>'+factura.serie+'-'+('000000'+factura.numeracion).slice(-6)+'</td>';
                            string += '<td>'+factura.f_emision.substr(8,2)+'/'+factura.f_emision.substr(5,2)+'/'+factura.f_emision.substr(0,4)+'</td>';
                            string += '<td>'+factura.f_vencimiento.substr(8,2)+'/'+factura.f_vencimiento.substr(5,2)+'/'+factura.f_vencimiento.substr(0,4)+'</td>';
                            string += '<td>'+factura.razon_social+'</td>';
                            string += '<td>'+$moneda[factura.moneda]+'</td>';
                            string += '<td>'+parseFloat(factura.total).toFixed(2)+'</td>';
                            string += '<td>'+$estado[factura.estado_doc]+'</td></tr>';
                    });
                } else {
                    string += '<tr><td colspan="7" style="text-align:center;">No se han realizado facturas para esta orden de compra</td></tr>';
                }
                $('#listaFact').html('').append(string);
        ...
        $('#btn_guardar').click(function(event){
            ...
            /*if( guias_select === null ){
                swal({
                    title: "Upss!",
                    text: "Debes agregar Guía/s de Compra correspondiente/s",
                    confirmButtonColor: "#66BB6A",
                    type: "error"
                },function(){
                    //window.location.reload();
            ...
            if(f_vencimiento.length == 0){
                swal({
                    title: "Upss!",
                    text: "Debes agregar Fecha de Vencimiento",
                    confirmButtonColor: "#66BB6A",
                    type: "error"
                },function(){
                    $('#btn_guardar').prop( "disabled", false );
            ...
            $.post(currentLocation+'crear_factura_compra',{idproveedor:idproveedor,serie:serie, numeracion:numeracion, igv:igv,subtotal:subtotal,descuento:descuento,medida_venta:medida_venta,total:total,comentarios:comentarios,f_emision:f_emision,f_vencimiento:f_vencimiento,productos:json_prod,paga:paga,vuelto:vuelto, guias_select:guias_select, moneda:moneda , id_orden_comprah:id_orden_comprah},function(data){
                console.log(data);
                var mensaje;
                ...
                if(mensaje === 200){
                    ...
                    swal({
                        ...
                        window.open(currentLocation+"listado_factura_compra");
                        window.location.reload(); 
                    });
                    return;
                }else if(mensaje === 500){
                    ...
                    swal({
                        title: "Error..!",
                        text: "No se puede guardar la factura, intentalo de nuevo luego.",
                        confirmButtonColor: "#66BB6A",
                        type: "error"
                    },function(){
                        $('#btn_guardar').prop( "disabled", false );
