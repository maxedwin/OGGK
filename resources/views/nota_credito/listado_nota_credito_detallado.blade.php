...
@section('titulo')
    <h4><i class="glyphicon glyphicon-usd position-left"></i> <span class="text-semibold">Listado de Devoluciones</span></h4>
@stop
...
@section('breadcrumb')
    ...
    <!-- <li>Inventario</li> -->
    <li class="active"><a href="/listado_caja"></i>Devoluciones</a></li>
@stop
<!-- MENU AUXLIAR -->
            ...
            <table class="table datatable-column-search-inputs dataTable table-hover dataTable no-footer" style="overflow-x:auto;" id="cliente_table">
                ...
                @foreach ($cajas as $caja)
                    <tr id="tr_detalle">
                        ...
                        <td>{{ $caja->nombre }}</td>
                        <td>{{ $caja->cantidad }}</td>
                        <td>{{ $caja->precio_unit }}</td>
                        <td>{{ $caja->precio_total }}</td>

                        <td> {{ $caja->name }} {{ $caja->lastname }}</td>
                        <?PHP  if($caja->estado_doc == 0) {
                            echo '<td> Efectuada </td>';
                        }      else{
                            echo '<td> Anulada </td>';
                        }  ?>
