...
@section('breadcrumb')
    ...
    <li class="active">Crear Nota de Crédito - Devolucione PDFFFF</li>
@stop
<!-- MENU AUXLIAR -->
...
<script src="{{ URL::asset('/javascript/cal.js') }}" type="text/javascript"></script>
   ...
   <script>
        ...
        $('#factbol_search').keyup(function(event) {
            var query = $(this).val();
            if(query.length >  0){
                $.get(currentLocation+"buscarFTNumpdf?query="+query+"", function( data ) {
                    $('#prueba').html('');
        ...
        $( "#prueba" ).on( "click","#item_to_add2", function() {
            ...
            $.get(currentLocation+"buscarFTTodopdf?query="+query+"", function( data ) {
                var obj = JSON.parse(data);
